...
import me.codeleep.victor.infra.voice.volcengine.protocol.HeaderSizeBits;
import me.codeleep.victor.infra.voice.volcengine.protocol.MsgTypeFlagBits;
import me.codeleep.victor.infra.voice.volcengine.protocol.SerializationBits;
import me.codeleep.victor.infra.voice.volcengine.protocol.VersionBits;
...
public class TtsMessage {
    ...
    public TtsMessage(TtsMsgType type, MsgTypeFlagBits flag) {
        this.type = type;
        this.flag = flag;
    }

    public static TtsMessage unmarshal(byte[] data) throws Exception {
        ByteBuffer buffer = ByteBuffer.wrap(data);

        byte type_and_flag = data[1];
        TtsMsgType type = TtsMsgType.fromValue((type_and_flag >> 4) & 0x0F);
        ...
        TtsMessage message = new TtsMessage(type, flag);
        ...
        if (flag == MsgTypeFlagBits.WITH_EVENT) {
            ...
            if (type != TtsMsgType.ERROR && !(message.event == EventType.START_CONNECTION
